...
public class PanelPrincipalFiltrado extends JPanel {
    ...
    public PanelPrincipalFiltrado(ViewFacade vf) {
        this.vf = vf;
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        try {
            ...
            JLabel bannerText = new JLabel("Resultados de búsqueda");
    ...
    // Método para mostrar productos filtrados
    public void mostrarProductosFiltrados(
        ...
        // Convertir el texto de búsqueda a minúsculas para hacer la búsqueda insensible a mayúsculas
        ...
        if (!productosAlimentos.isEmpty()) {
            ...
            for (int i = 0; i < productosAlimentos.size(); i++) {
                ...
                if (coincideBusqueda(producto, busquedaLower)) {
                    ...
                    hayProductosEnCategoria = true;
                }
            }
            
            if (hayProductosEnCategoria) {
        ...
        // Si no hay productos que coincidan, mostrar un mensaje
        if (panelProductos.getComponentCount() == 0) {
            JPanel panelNoResultados = new JPanel();
            panelNoResultados.setLayout(new FlowLayout());
            panelNoResultados.setBackground(Color.WHITE);
            JLabel lblNoResultados = new JLabel("No se encontraron productos que coincidan con: " + textoBusqueda);
            lblNoResultados.setFont(new Font("Arial", Font.BOLD, 16));
            panelNoResultados.add(lblNoResultados);
            panelProductos.add(panelNoResultados);
        }
    ...
    // Método para verificar si un producto coincide con la búsqueda
    private boolean coincideBusqueda(Producto producto, String busquedaLower) {
        // Verificar en el nombre
        if (producto.getNombre() != null && producto.getNombre().toLowerCase().contains(busquedaLower)) {
            return true;
        }
        // Verificar en la marca
        if (producto.getMarca() != null && producto.getMarca().toLowerCase().contains(busquedaLower)) {
            return true;
        }
        // Verificar en la descripción
        if (producto.getDescripcion() != null && producto.getDescripcion().toLowerCase().contains(busquedaLower)) {
            return true;
        }
        
        // Dependiendo del tipo de producto, verificar en campos específicos
        if (producto instanceof Celular) {
            Celular celular = (Celular) producto;
            if (celular.getColor() != null && celular.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (celular.getMemoriaInterna() > 0 && String.valueOf(celular.getMemoriaInterna()).contains(busquedaLower)) {
                return true;
            }
            if (celular.getMemoriaRam() > 0 && String.valueOf(celular.getMemoriaRam()).contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof AlimentoYBebida) {
            AlimentoYBebida alimento = (AlimentoYBebida) producto;
            if (alimento.getTipoEnvase() != null && alimento.getTipoEnvase().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (alimento.getUnidadEnvase() > 0  && String.valueOf(alimento.getUnidadEnvase()).contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Construccion) {
            Construccion construccion = (Construccion) producto;
            if (construccion.getModelo() != null && construccion.getModelo().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (construccion.getMaterial() != null && construccion.getMaterial().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (construccion.getColor() != null && construccion.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof DeporteYFitness) {
            DeporteYFitness deporte = (DeporteYFitness) producto;
            if (deporte.getColor() != null && deporte.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (deporte.getMaterial() != null && deporte.getMaterial().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Electrodomestico) {
            Electrodomestico electrodomestico = (Electrodomestico) producto;
            if (electrodomestico.getModelo() != null && electrodomestico.getModelo().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (electrodomestico.getColor() != null && electrodomestico.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (electrodomestico.getVoltaje() > 0 && String.valueOf(electrodomestico.getVoltaje()).contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Juguete) {
            Juguete juguete = (Juguete) producto;
            if (juguete.getColor() != null && juguete.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (juguete.getMaterial() != null && juguete.getMaterial().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (juguete.getRangoDeEdad() != null && juguete.getRangoDeEdad().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Mascota) {
            Mascota mascota = (Mascota) producto;
            if (mascota.getTipoAnimal() != null && mascota.getTipoAnimal().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (mascota.getRaza() != null && mascota.getRaza().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (mascota.getColor() != null && mascota.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (mascota.getFormatoDeVenta() != null && mascota.getFormatoDeVenta().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Farmacia) {
            Farmacia farmacia = (Farmacia) producto;
            if (farmacia.getLaboratorio() != null && farmacia.getLaboratorio().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (farmacia.getFormatoMedicamento() != null && farmacia.getFormatoMedicamento().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (farmacia.getFormatoDeVenta() != null && farmacia.getFormatoDeVenta().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Moda) {
            Moda moda = (Moda) producto;
            if (moda.getColor() != null && moda.getColor().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (moda.getTalla() != null && moda.getTalla().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (moda.getMaterial() != null && moda.getMaterial().toLowerCase().contains(busquedaLower)) {
                return true;
            }
        } else if (producto instanceof Vehiculo) {
            Vehiculo vehiculo = (Vehiculo) producto;
            if (vehiculo.getMarca() != null && vehiculo.getMarca().toLowerCase().contains(busquedaLower)) {
                return true;
            }
            if (vehiculo.getAnio() > 0 && String.valueOf(vehiculo.getAnio()).contains(busquedaLower)) {
                return true;
            }
            if (vehiculo.getKilometraje() > 0 && String.valueOf(vehiculo.getKilometraje()).contains(busquedaLower)) {
                return true;
            }
        }
